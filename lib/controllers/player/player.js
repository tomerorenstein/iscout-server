    ...
    playerDAL.getPlayerInfo(playerId, function (err, results) {
        ...
        if (err){
            ...
            if (isExtraData) {
                ...
                resJson.img =
                    (results[0].img != null) ? (new Buffer(results[0].img, 'binary').toString('base64')) : "";
                        ...
                        playerDAL.getPlayerInfo(playerIdRes[0].player_id, function (err, results) {
                            ...
                            if (err){
                                ...
                                let resJson = {
                                    ...
                                    country: results[0].country,
                                    team: results[0].team,
                                    videos_url: results[0].urls,
                                    img: (results[0].img != null) ?
                                        (new Buffer(results[0].img, 'binary').toString('base64')) : ""
                                };
...
exports.uploadVideo = function (request, reply) {
    ...
    playerDAL.insertVideoUrl(request.payload.player_id, request.payload.youtube_url, function (err, results) {
